...
interface ElectronAPI {
  ...
  quitApp: () => void;
  /** Listen for system idle state changes (active | paused | reset) */
  onIdleState?: (callback: (state: 'active' | 'paused' | 'reset') => void) => () => void;
  /** Face profile IPC — electron-store backed, never synced to cloud */
  faceProfiles: {
    save: (email: string, name: string, embedding: number[] | Float32Array) => Promise<boolean>;
    loadAll: () => Promise<Record<string, { name: string; embedding: number[]; enrolledAt: string }>>;
