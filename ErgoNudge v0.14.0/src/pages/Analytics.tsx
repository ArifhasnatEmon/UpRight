...
<head>
<meta charset="UTF-8">
<title>ErgoNudge Weekly Report — ${format(new Date(), 'MMM d, yyyy')}</title>
<style>
...
<div class="container">
  ...
  <div class="footer">Generated by ErgoNudge · ${format(new Date(), 'MMM d, yyyy h:mm a')}</div>
</div>
</body>
</html>`;
            ...
            a.href = url;
            a.download = `ergonudge-weekly-${format(new Date(), 'yyyy-MM-dd')}.html`;
            a.click();
        ...
        <button
          onClick={() => {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `ergonudge-export-${format(new Date(), 'yyyy-MM-dd')}.json`;
            a.click();
            URL.revokeObjectURL(url);
          }}
