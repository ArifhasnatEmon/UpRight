...
import { User as UserIcon, Zap, Lock, Trophy, LogOut, Edit2, Check, X, Star, Target, Flame, Camera, RefreshCw, Cloud, CloudOff, Loader, ScanFace, UserCheck, AlertCircle, Trash2 } from 'lucide-react';
...
export const Profile: React.FC<ProfileProps> = ({ user, logs, settings, setSettings, onLogout, onUpdateName, currentUserEmail, syncNow, isSyncing = false, lastSyncAt, syncError, enrollRef }) => {
  ...
  const loadFaceStatus = useCallback(async () => {
    if (!currentUserEmail || !window.electronAPI?.faceProfiles) {
      console.info('[FaceStatus] Skipped — no email or no Electron IPC');
      return;
    }
    try {
      const profiles = await window.electronAPI.faceProfiles.loadAll();
      console.info('[FaceStatus] Loaded profiles, keys:', Object.keys(profiles), 'looking for:', currentUserEmail);
      const p = profiles[currentUserEmail];
      if (p && p.enrolledAt) {
        setIsFaceEnrolled(true);
        setFaceEnrolledAt(p.enrolledAt);
      } else {
        setIsFaceEnrolled(false);
        setFaceEnrolledAt(null);
      }
    } catch (err) {
      console.warn('[FaceStatus] Error loading profiles:', err);
      setIsFaceEnrolled(false);
    }
  }, [currentUserEmail]);
  ...
  const handleEnrollFace = async () => {
    ...
    const fn = enrollRef?.current;
    if (!fn) {
      setEnrollStatus('error');
      setFaceEnrollError('Camera not ready. Make sure dashboard is active.');
      return;
    }

    const result = await fn(currentUserEmail, user.name);
    if (result.success) {
      setEnrollStatus('success');
      // Optimistic update — immediately show enrolled status without
      // waiting for the IPC round-trip, which may fail silently.
      setIsFaceEnrolled(true);
      setFaceEnrolledAt(new Date().toISOString());
      // Also verify via IPC (will correct with accurate data if needed)
      loadFaceStatus();
      setTimeout(() => setEnrollStatus('idle'), 3000);
    } else {
      setEnrollStatus('error');
      setFaceEnrollError(result.error ?? 'Scan failed. Make sure your face is well-lit and centred.');
    }
  };

  const handleRemoveFace = async () => {
    if (!currentUserEmail || !window.electronAPI?.faceProfiles) return;
    try {
      await window.electronAPI.faceProfiles.delete(currentUserEmail);
      loadFaceStatus();
    } catch (err) {
      console.warn('Failed to remove face profile:', err);
    }
  };
  ...
  const handleCropSave = () => {
    ...
    if (settings.cloudSyncEnabled && syncNow) {
      setTimeout(() => syncNow(), 500); // slight delay so localStorage writes first
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6" role="region" aria-label="Profile">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="glass rounded-[2rem] p-6 space-y-5">
          <h3 className="font-display font-bold text-base flex items-center gap-2 text-fg">
            <Cloud className="w-4 h-4 text-brand-600 dark:text-brand-400" /> Data & Sync
          </h3>
          
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold text-sm text-fg">Cloud Backup</p>
              <p className="text-[11px] text-fg-faint mt-0.5">Securely sync your posture history across devices</p>
            </div>
            <button
              onClick={toggleCloudSync}
              className={cn("w-10 h-5 rounded-full relative transition-colors shrink-0", settings.cloudSyncEnabled ? "bg-brand-500" : "bg-edge")}
              role="switch"
              aria-checked={settings.cloudSyncEnabled}
              aria-label="Toggle cloud sync"
            >
              <div className={cn("absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all shadow-sm", settings.cloudSyncEnabled ? "right-0.5" : "left-0.5")} />
            </button>
          </div>
          
          <div className="flex flex-col gap-3 p-4 rounded-xl bg-inset border border-edge-subtle">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                {settings.cloudSyncEnabled ? (
                  syncError ? (
                    <div className="p-2 bg-red-500/10 rounded-xl"><CloudOff className="w-4 h-4 text-red-500" /></div>
                  ) : isSyncing ? (
                    <div className="p-2 bg-brand-500/10 rounded-xl"><Loader className="w-4 h-4 text-brand-500 animate-spin" /></div>
                  ) : (
                    <div className="p-2 bg-emerald-500/10 rounded-xl"><Cloud className="w-4 h-4 text-emerald-500" /></div>
                  )
                ) : (
                  <div className="p-2 bg-neutral-500/10 dark:bg-white/5 rounded-xl"><CloudOff className="w-4 h-4 text-fg-faint" /></div>
                )}
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-fg">
                    {settings.cloudSyncEnabled ? (syncError ? 'Sync Failed' : isSyncing ? 'Syncing...' : 'Connected') : 'Local Mode'}
                  </span>
              ...
              {settings.cloudSyncEnabled && syncNow && (
                <button
                  ...
                  disabled={isSyncing}
                  className={cn(
                    "text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-all",
                    isSyncing
                      ? "bg-edge text-fg-faint cursor-not-allowed"
                      : "bg-tint-brand text-brand-700 hover:bg-brand-500 hover:text-white dark:bg-brand-500/20 dark:text-brand-300 dark:hover:bg-brand-500 dark:hover:text-white"
                  )}
                  aria-label="Sync now"
                >
            ...
            <div className="h-px w-full bg-edge-subtle my-1" />
            
            <div className="flex items-start gap-2.5">
              <Lock className="w-3.5 h-3.5 text-brand-500 mt-0.5 shrink-0" />
              <p className="text-[10px] text-fg-faint leading-relaxed">
                <strong>Privacy Guaranteed:</strong> Your camera feed is never recorded. All AI posture analysis runs 100% locally on your device before minimal text data is synced.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="glass rounded-[2rem] p-6 space-y-4 col-span-2">
          <h3 className="font-display font-bold text-base flex items-center gap-2 text-fg">
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-inset border border-edge-subtle">
            <div className="flex items-center gap-3">
              <div className={cn("w-10 h-10 rounded-full flex items-center justify-center", isFaceEnrolled ? "bg-emerald-500/10 text-emerald-500" : "bg-neutral-500/10 text-neutral-500")}>
                {isFaceEnrolled ? <UserCheck className="w-5 h-5" /> : <ScanFace className="w-5 h-5" />}
              </div>
              <div>
                <p className="font-bold text-sm text-fg">
                  {isFaceEnrolled ? 'Face Enrolled' : 'Not Enrolled'}
                </p>
                {isFaceEnrolled && faceEnrolledAt && (
            ...
            <div className="flex items-center gap-3">
              {enrollStatus === 'scanning' ? (
                ...
                <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-xl">
                ...
                <>
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-colors"
                  >
      ...
      <div className="glass rounded-[2rem] p-8 space-y-6">
        ...
        <div className="space-y-6">
          ...
          {(() => {
            const unlockedDefs = ALL_ACHIEVEMENTS.filter(def => user.achievements.some(a => a.id === def.id));
              ...
              <div className="grid grid-cols-2 gap-3">
                {unlockedDefs.map(def => {
                  ...
                  return (
                    <div
                      key={def.id}
                      className="flex items-center gap-3 p-4 rounded-2xl border transition-all bg-tint-brand border-brand-100 dark:border-brand-500/20 shadow-sm"
                    >
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0 bg-card shadow-sm">
                        {def.icon}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-sm truncate text-fg">
                          {def.title}
                        </p>
                        <p className="text-[10px] text-fg-secondary leading-tight mt-0.5">{def.description}</p>
                        <p className="text-[9px] text-brand-500 dark:text-brand-400 mt-1 font-bold tracking-wide uppercase">
          ...
          {(() => {
            const lockedDefs = ALL_ACHIEVEMENTS.filter(def => !user.achievements.some(a => a.id === def.id));
            if (lockedDefs.length === 0) return null;
            return (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <h4 className="font-display font-bold text-sm text-fg-muted">Locked</h4>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {lockedDefs.map(def => (
                    <div
                      key={def.id}
                      className="flex items-start gap-2 p-3 rounded-xl border bg-inset border-edge-subtle opacity-60 hover:opacity-80 transition-opacity grayscale hover:grayscale-0"
                    >
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center text-lg shrink-0 bg-edge-subtle">
                        {EMOJI_LOCK}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-xs truncate text-fg-faint">
                          {def.title}
                        </p>
                        <p className="text-[9px] text-fg-faint leading-tight mt-0.5 line-clamp-2">{def.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
